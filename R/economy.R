...
#' @examples res.list <- get_economic_indicators()

get_economic_indicators <- function (verbose=TRUE) {

  ## Economic indicators; Hel, Esp, Van, Kau, Koko maa; years 1998-2010
  if (verbose)
    message("Reading economic indicators\nHelsinki, Espoo, Vantaa, Kauniainen, Finland\nYears 1998-2010.")
  ...
  # Read metadata
  met1.raw <- read.csv(csv.url, sep=";", header=FALSE, fileEncoding="ISO-8859-1", nrow=2, stringsAsFactors=FALSE)
  met2.raw <- read.csv(csv.url, sep=";", header=FALSE, fileEncoding="ISO-8859-1", skip=165, stringsAsFactors=FALSE)
  return(list(indicators=ec.ind, metadata=met1.raw$V1, notes=met2.raw$V1))
}
